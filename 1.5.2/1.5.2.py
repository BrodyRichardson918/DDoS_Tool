...
class LoadTester:
    ...
    def __init__(self, targets, request_size=1024, concurrency=100, timeout=30, protocol="http", proxies=None, duration=None, deadhand_mode=False):
        ...
        for target in targets:
            if not target.replace('.', '').isdigit():
                try:
                    ...
                    self.targets.append(f"{protocol}://{ip}")
                except socket.gaierror:
                    print(Fore.RED + f"无法解析目标地址: {target}, 将跳过此目标")
            else:
        ...
        logger.info(f"初始化负载测试器: 目标={self.targets}, 协议={protocol}, 并发={concurrency}, 死手模式={deadhand_mode}")
...
您确定要继续吗？"""
    ...
    def _scan_target_ports(target):
        ...
        try:
            ...
            for info in port_info:
                print(Fore.CYAN + f"  端口 {info['port']}: {info['service']} {info['product']} {info['version']} 风险评分: {info['risk_score']}")
                
            # 返回风险最高的前3个端口    
            return [str(info['port']) for info in port_info[:3]]
        except Exception as e:
            print(Fore.RED + f"端口扫描出错: {str(e)}")
            return []
    ...
    def config():
        ...
        with open(OperationConsole.CONFIG_FILE, 'w') as f:
            json.dump({'targets': targets, 'targets_ports': targets_ports, 'config': config}, f)
                ...
                while retry < MAX_RETRY:
                    try:
                        ...
                        if retry == MAX_RETRY:
                            ...
                            choice = input(Fore.YELLOW + "请检查网络连接,或直接输入IP。是否跳过这个目标? (y/N): ").strip().lower()
                            if choice == 'y':
            ...
            for target, ports in targets_ports.items():
                if ports:
                    ...
                    if port == 80 or port == 443:
                        ...
                        config['protocol'] = 'tcp'  # ftp 使用 tcp
                    elif port == 53:
                        config['protocol'] = 'udp'  # dns 使用 udp
                    else:
                        config['protocol'] = 'tcp'
                        
                    print(Fore.GREEN + f"目标 {target} 将使用 {config['protocol']} 协议攻击 {port} 端口")
                    
                    # 调整目标 URL
                    if '://' not in target:
                        target = f"{config['protocol']}://{target}:{port}"
                    else:
                        target = f"{target.split('://')[0]}://{target.split('://')[1].split('/')[0]}:{port}"
